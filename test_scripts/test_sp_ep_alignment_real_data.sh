...
MASTER_PORT_DP="${MASTER_PORT_DP:-29601}"
MASTER_PORT_SP_EP="${MASTER_PORT_SP_EP:-29602}"
MASTER_PORT_SINGLE="${MASTER_PORT_SINGLE:-29603}"
...
OUTPUT_FIG="${OUTPUT_FIG:-test_scripts/loss_alignment_real_data.png}"

# Real data config (consumed by pretrain_example.py)
...
export HF_DATASET_REPO HF_DATASET_NAME HF_SPLIT HF_TOKENIZER
export HF_MAX_SAMPLES HF_MIN_CHARS HF_ADD_EOS_TOKEN HF_SHUFFLE_BUFFER
...
"
if [[ -n "$MAX_STEPS" ]]; then
  COMMON_TRAINING_ARGS="$COMMON_TRAINING_ARGS --max_steps $MAX_STEPS"
fi
...
echo "========================================"
echo "RUNNING SINGLE CARD (REAL DATA)"
echo "========================================"

EXP_NAME_SINGLE="test_alignment_real_single_${RUN_TAG}"
BATCH_SIZE_PER_SINGLE=$BATCH_SIZE
SEP_SIZE_SINGLE=1
...
torchrun \
  ...
  pretrain_example.py \
  --exp_name "$EXP_NAME_SINGLE" \
  --batch_size "$BATCH_SIZE_PER_SINGLE" \
  $COMMON_TRAINING_ARGS \
  --sep_size "$SEP_SIZE_SINGLE" \
  $MODEL_ARGS

echo "========================================"
echo "RUNNING STANDARD DP (REAL DATA)"
echo "========================================"

EXP_NAME_DP="test_alignment_real_dp_${RUN_TAG}"
...
echo "========================================"
echo "RUNNING SP + EP (REAL DATA)"
echo "========================================"

EXP_NAME_SP_EP="test_alignment_real_sp_ep_${RUN_TAG}"
SEP_SIZE=2
...
LOG_SINGLE_DIR=$(find "$LOG_DIR" -name "${EXP_NAME_SINGLE}*" -type d | head -n 1 || true)
...
if [[ -z "$LOG_SINGLE_DIR" || -z "$LOG_DP_DIR" || -z "$LOG_SP_EP_DIR" ]]; then
  echo "Could not find experiment directories under $LOG_DIR."
  exit 1
fi

LOG_SINGLE="$LOG_SINGLE_DIR/log.jsonl"
LOG_DP="$LOG_DP_DIR/log.jsonl"
LOG_SP_EP="$LOG_SP_EP_DIR/log.jsonl"

if [[ ! -f "$LOG_SINGLE" || ! -f "$LOG_DP" || ! -f "$LOG_SP_EP" ]]; then
...
python test_scripts/compare_loss.py "$LOG_SINGLE" "Single GPU" "$LOG_DP" "DP" "$LOG_SP_EP" "SP+EP" "$OUTPUT_FIG"
...
echo "Tailing single / dp / sp+ep loss:"
paste \
  <(grep '"stage": "train"' "$LOG_SINGLE" | python -c "import sys,json; [print(f'single step={json.loads(l)[\"step\"]} loss={json.loads(l)[\"loss\"]}') for l in sys.stdin]" 2>/dev/null | head -20) \
  <(grep '"stage": "train"' "$LOG_DP"     | python -c "import sys,json; [print(f'dp     step={json.loads(l)[\"step\"]} loss={json.loads(l)[\"loss\"]}') for l in sys.stdin]" 2>/dev/null | head -20) \
  <(grep '"stage": "train"' "$LOG_SP_EP"  | python -c "import sys,json; [print(f'sp_ep  step={json.loads(l)[\"step\"]} loss={json.loads(l)[\"loss\"]}') for l in sys.stdin]" 2>/dev/null | head -20)
